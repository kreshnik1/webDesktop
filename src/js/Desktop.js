...
function Desktop() {
    ...
    this.clickX = 0;
    this.clickY = 0;
    this.serialNumber = 0;//counting the windows
    this.zIndex = 0;
    this.offsetX = 1;
    this.offsetY = 1;
    ...
    if (element.classList.contains("window")) {
        ...
        if (event.target.classList.contains("window-top")) {
            if (!event.target.parentNode.classList.contains("maximized")) {
                this.clickX = event.clientX - this.activeWindow.x;//event.clientX coordinates
...
Desktop.prototype.mouseMove = function(event) {
 ...
 /**
 * Function to handle which window is clicked
 * to exit , minimize or maximize
 * @param event
 */
